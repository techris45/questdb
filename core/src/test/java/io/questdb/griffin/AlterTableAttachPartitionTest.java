...
public class AlterTableAttachPartitionTest extends AbstractAlterTableAttachPartitionTest {
    ...
    public void testAttachPartitionInWrongDirectoryName() throws Exception {
        assertMemoryLeak(() -> {
            try (
                ...
                TableToken srcToken = createPopulateTable(
                        1,
                        src.col("l", ColumnType.LONG)
                                .col("i", ColumnType.INT)
                                .timestamp("ts"),
                ...
                TableToken dstToken = CairoTestUtils.create(
                        dst.col("i", ColumnType.INT)
                                .col("l", ColumnType.LONG)
                                .timestamp("ts"));

                copyPartitionToAttachable(srcToken, "2020-01-01", dstToken, "COCONUTS");
    ...
    public void testAttachPartitionsDeletedColumnFromSrc() throws Exception {
        assertMemoryLeak(() -> {
            try (TableModel src = new TableModel(configuration, testName.getMethodName() + "_src", PartitionBy.DAY);
                ...
                TableToken dstToken = CairoTestUtils.create(dst.timestamp("ts")
                        .col("i", ColumnType.INT)
                        .col("l", ColumnType.LONG)
                        .col("s", ColumnType.SYMBOL).indexed(true, 128)
                        .col("str", ColumnType.STRING)
                ...
                copyPartitionToAttachable(srcToken, "2022-08-02", dstToken, "2022-08-02");
    ...
    public void testAttachPartitionsDetachedHasExtraColumn() throws Exception {
        assertMemoryLeak(() -> {
            try (TableModel src = new TableModel(configuration, "src48", PartitionBy.DAY);
                ...
                copyPartitionToAttachable(srcToken, "2022-08-01", dstToken, "2022-08-01");
    ...
    public void testDetachAttachDifferentPartitionTableReaderReload() throws Exception {
        ...
        assertMemoryLeak(() -> {
            try (TableModel src = new TableModel(configuration, "src47", PartitionBy.DAY);
                ...
                TableToken dstToken = createPopulateTable(
                        1,
                        dst.col("l", ColumnType.LONG)
                                .col("i", ColumnType.INT)
                                .col("str", ColumnType.STRING)
                ...
                try (TableReader dstReader = newTableReader(configuration, dst.getTableName())) {
                    ...
                    try (TableWriter writer = getWriter(dst.getTableName())) {
                        ...
                        copyPartitionToAttachable(srcToken, "2020-01-09", dstToken, "2020-01-09");
    ...
    void attachFromSrcIntoDst(TableModel src, TableModel dst, String... partitionList) throws SqlException, NumericException {
        ...
        engine.clear();
        path.of(configuration.getRoot()).concat(engine.getTableToken(src.getName()));
        int pathLen = path.length();
        other.of(configuration.getRoot()).concat(engine.getTableToken(dst.getName()));
        ...
        try (TableReader reader = getReader(dst.getName())) {
        ...
        engine.clear();
        try (TableWriter writer = getWriter(engine, dst.getName())) {
