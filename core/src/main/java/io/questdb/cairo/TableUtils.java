...
public final class TableUtils {
    ...
    public static void createTable(
        ...
        createTable(configuration, memory, path, false, structure, ColumnType.VERSION, tableId, dirName);
    }
    ...
    public static void createTable(
        ...
        createTable(configuration, memory, path, pathIsOtherVolume, structure, ColumnType.VERSION, tableId, dirName);
    }
    ...
    public static void createTable(
        ...
        createTable(ff, root, mkDirMode, memory, path, pathIsOtherVolume, structure, tableVersion, tableId, dirName);
    }
    ...
    public static void createTable(
        ...
        createTable(ff, root, mkDirMode, memory, path, pathIsOtherVolume, dirName, structure, tableVersion, tableId);
    }
    ...
    public static void createTable(
        ...
        LOG.debug().$("create table [name=").$(tableDir).I$();
        ...
        path.concat(tableDir);
        final int rootLen = path.length();

        if (ff.mkdirs(path.slash$(), mkDirMode) != 0) {
            throw CairoException.critical(ff.errno()).put("could not create [dir=").put(path).put(']');
        }
        if (pathIsOtherVolume && ff.softLink(path.trimTo(rootLen).$(), Path.getThreadLocal2(root).concat(tableDir).$()) != 0) {
            throw CairoException.critical(ff.errno()).put("could not create soft link [src=").put(path).put(", tableDir=").put(tableDir).put(']');
        }
